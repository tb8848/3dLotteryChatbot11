...
function ding3Code(lsId){
    ...
    codeSet.sort(sortByCharAsc)
    codeSet = initCodeInfo(codeSet,lsId)
    return codeSet;
}
...
function sortByDigitAsc(n1,n2){
    return n1-n2;
}


function sortByCharAsc(n1,n2){
    ...
    for(var i=0;i<10;i++){
        for(var j=0;j<10;j++){
            for(var k=0;k<10;k++){
                ...
                var c2 = i+"X"+k;
                var c3 = "X"+j+k;
                if(!codeSet.includes(c1)){
                    codeSet.push(c1)
                }
                if(!codeSet.includes(c2)){
                    codeSet.push(c2)
                }
                if(!codeSet.includes(c3)){
                    codeSet.push(c3)
                }
            }
        }
...
function initCodeInfo(clist,lsId){
    var codesList = [];
    clist.forEach((code,idx)=>{
        codesList.push({
            code:code,
            xian:0,
            maxLoss:0,
...
// function ding3Code(numStr,len,xianFlag){
//     var codes = [];
