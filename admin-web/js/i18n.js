            ...
            if (value.length == 1 && typeof (value[0]) == "string") {
                ...
                if(params.length>0){
                    for(var i=0,len=params.length;i<len;i++){
                        var holderIdx = v.indexOf("#");
                        if(holderIdx>-1){
                            var phalf = v.substring(0,holderIdx+2);
                            var shalf = v.substring(holderIdx+2,v.length-1);
                            phalf = phalf.replace(/^#\d$/,params[i])
                            v = phalf+shalf;
                        }
                    }
                }
...
layui.define(["i18n"], function (exports) {
    ...
    i18np.loadProperties = function (module) {
        ...
        if (baseLang == "undefined" || baseLang == "" || baseLang == null) {
            ...
            layui.data('langTab', { key: 'langType', value: baseLang });
        }
        ...
        if (module == "" || module == null || module == undefined) {
            i18nPath = 'js/i18n/';
        }else if (module == 'index' || module == 'main' || module == 'subAccount_main') {
            i18nPath = 'js/i18n/'+module;
        }else{
            if(module.indexOf("subMain")>-1 || module.indexOf("subAccount")>-1 || module.indexOf("admin_vip")>-1){
                module  = module.split("-")[1];
                i18nPath = '../../js/i18n/'+module;
            }else if(module.indexOf("pwd")>-1){
                module  = module.split("-")[1];
                i18nPath = 'js/i18n/'+module;
            }else{
                i18nPath = '../js/i18n/'+module;
