...
const vm = new Vue({
    ...
    methods: {
        ...
        findStopBuyCodes(){
            ...
            console.log("============停押 change ==",_this.stopCodeChangeList)
        },
        toPrint(){
        ...
        initDataList() {
            ...
            $.ajax({
                ...
                url:HOST +'admin/shipan/lmList?lotteryType=1',
                type: 'get',
                success: function (res) {
                    if (res.code == 0) {
                        ...
                        _this.settingList.forEach((item,idx)=>{
                            _this.lmFuzhiMap[item.id]=item.fuzhi;
                            //$("#lm-fz-"+item.id).html(item.fuzhi);
                            item.lotterySettingList.forEach((it,idx1)=>{
                                it.avgOdds = 0;
                            })
                        })
                        // if(_this.lmId==4){
                        //     _this.getBuyList4(); //此接口会返回停押号码列表
                        // }else{
                        //     _this.getDatas();
                        // }
                        _this.getDatas();
        ...
        getBuyList4(){
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res.code == 0) {
                        if(res.data.list1!=null){
                            _this.buyList = res.data.list1;
        ...
        getBuyList(){
            ...
            $.ajax({
                //url: HOST + 'admin/drawbuy/xupan/list',
                url: HOST + 'admin/shipan/listAll?lmId='+_this.lmId,
                type: 'get',
                success: function (res) {
                    layer.closeAll();
                    if(_this.lmId!=4){
        ...
        upOddsOfCodes4(newOdds){
            var pageData = this.settingCodesList[0].pageData
            ...
            this.settingCodesList[0].pageData = pageData;
        },
        cancelOddsChange() {
        ...
        createSettingCodes4() {
            ...
            //
            //
            //     _this.buyList.forEach((cc,ii)=>{
            //         var checkStopCode = _this.__isStopCode(cc.code,cc.lsid)
            //         cc.selected = checkStopCode;
            //         cc.isStopBuy = checkStopCode?1:0;
            //         //cc.selected = cc.isStopBuy==1;
            //     })
            //     var dlist = createCodesForDing(4, item.bettingRule, _lsId);
            //     item.avgOdds = lsAvgOdds;
            //     var codesInfo = {
        ...
        getPagerData4(){
            ...
            if (this.settingCodesList.length == 1) {
                ...
                var pgData = codeList.slice(st, et);
                if(!_this.fillOk){
                    pgData = this.__updateCodesData4(pgData,0,_this.lsList[0])
                }
        ...
        // getPagerData4(totalLen) {
        //     var _this = this;
        //     _this.stopCodeChangeList = [];
        //     if (this.settingCodesList.length == 1) {
        //         var findIdx = -1;
        //         var codeList = this.settingCodesList[0].codesList;
        //         this.pager.total = totalLen;
        //
