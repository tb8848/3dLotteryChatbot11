...
const vm = new Vue({
    ...
    methods: {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res.code == 0) {
                        ...
                        _this.stopBuyList = res.data.stopBuyCodesList;
                        _this.drawCodeList = res.data.drawCodeList;
                        console.log("=======11111111",_this.drawCodeList)
                        if(_this.lmId==4){
        ...
        __isDrawCode(code,lsid){
            var exist = false;
            if(this.drawCodeList!=null) {
                this.drawCodeList.forEach((item, idx) => {
                    if (item == code) {
                        exist = true;
                    }
                })
        ...
        createSettingCodes() {
            ...
            this.lsList.forEach((ls, idx) => {
                ...
                switch (_this.lmId) {
                    case 1:
                        dlist = createCodesForDing(1, ls.bettingRule, _lsId);
                        break;
                    case 2:
                        dlist = createCodesForDing(2, ls.bettingRule, _lsId);
                        break;
                    case 3:
                        dlist = createCodesForDing(3, ls.bettingRule, _lsId);
                        break;
                    case 4:
                        dlist = createCodesForDing(4, ls.bettingRule, _lsId);
                        break;
                    case 5:
        ...
        getPagerDataAtFullScreen(){ //全屏模式下的分页
            ...
            this.settingCodesList.forEach((item,idex)=>{
                if(item.lsid == _this.lsId){
                    ...
                    _this.lines =Math.ceil(pgData.length / 8);
