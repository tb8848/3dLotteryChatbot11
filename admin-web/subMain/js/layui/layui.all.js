...
layui.define(function (e) {
    ...
    u.prototype.type = function () {
        ...
        var n = a.pages > t ? Math.ceil((a.curr + (t > 1 ? 1 : 0)) / (t > 0 ? t : 1)) : 1, i = {
            ...
            count: '<span class="layui-laypage-count"> ' + i18np.prop("user.page.total") + a.count + i18np.prop("user.page.count") +" </span>",
            limit: function () {
                var e = ['<span class="layui-laypage-limits"><select lay-ignore>'];
                return layui.each(a.limits, function (t, n) {
                    e.push('<option value="' + n + '"' + (n === a.limit ? "selected" : "") + ">" + n + i18np.prop("user.page.count") + "/" + i18np.prop("user.page.page") + "</option>")
                }), e.join("") + "</select></span>"
            }(),
            refresh: ['<a href="javascript:;" data-page="' + a.curr + '" class="layui-laypage-refresh">', '<i class="layui-icon layui-icon-refresh"></i>', "</a>"].join(""),
            skip: function () {
                return ['<span class="layui-laypage-skip">' + i18np.prop("user.page.to") + i18np.prop("user.page.no"), '<input type="text" min="1" value="' + a.curr + '" class="layui-input">', i18np.prop("user.page.page") + '<button type="button" class="layui-laypage-btn">' + i18np.prop("user.add.msg.btn.ok") + '</button>', "</span>"].join("")
            }()
        };
