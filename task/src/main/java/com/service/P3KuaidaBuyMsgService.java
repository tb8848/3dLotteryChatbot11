            ...
            if (arr[0].equals("拖拉机") || arr[0].equals("三同号") || arr[0].equals("猜三同")
                ...
                for (String word : GlobalConst.keywords) {
                    ...
                    if (typePart.startsWith(word)) {
                        ...
                        String code = null;
                        matchSucc = true;
                        String bsgRegex = "[百][0-9]+[十][0-9]+[个][0-9]+";
                        if(typePart.matches(bsgRegex)){
                            code = typePart;
                        }else{
...
//                wechatApiService.sendMsg(player.getWxFriendId(),botUser.getWxId(),toMsg.getMsg());
    ...
    public void reportToPan(BotUser botUser,Player player,ChatRoomMsg msg,List<BuyRecord3DVO> buyList,BotUserPan botUserPan){
        ...
        if(reportToPanUrl.indexOf("aa.3d11aa.com")>-1){
            reportToPanUrl+=":9092";
        ...
        switch (reportRespData.getCode()){
            case 403: //盘口登录token已失效
                botUserPanService.clearInfo(2,botUser.getId());
            ...
            case 0:
                ...
                if(null!=dataList && dataList.size()>0){
                    ...
                    if(drawBuyRecordDAO.batchAddBuyCode(dataList)>0){
                        ...
                        String newmsg = "[课号]"+draw.getDrawId()+"\r\n"+msg.getMsg()+"\r\n交作业成功√√\r\n【份数】："+buyAmount+"\r\n"
    ...
    public void xiazhu(BotUser botUser, Player player,ChatRoomMsg fromMsg, List<BuyRecord3DVO> buyList) {
        ...
        if(player.getPretexting()==1 || player.getEatPrize()==1){
            ...
            Map<String,Object> buyResult = null;
            if("5".equals(lmId) || "13".equals(lmId)){
                buyResult = buyRecord3DService.buy3dHs(player,buyList,draw,-2,null,msg,2);
            }else{
                buyResult = buyRecord3DService.codesBatchBuy(player,buyList,draw,-2,null,msg,2);
            }
            if(buyResult.containsKey("playerBuyId")){
                ...
                BigDecimal points = playerService.getPoints(player.getId());
                String newmsg = "[P3课号]"+draw.getDrawId()+"\r\n"+msg+"\r\n交作业成功√√\r\n【份数】："+record.getBuyAmount()+"\r\n"
            ...
            if(player.getReportNet()==1){
                BotUserPan botUserPan = botUserPanService.getOneBy(2,botUser.getId());
                if(null == botUserPan){
                    //机器人不支持P3
