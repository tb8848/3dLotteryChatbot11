 ...
 */
@Service
public class BuyRecord3DService extends ServiceImpl<DrawBuyRecordDAO, DrawBuyRecord> {
    ...
    public Map<String,Object> hz(Player player,LotteryMethod lotteryMethod,
        ...
        BotUserOdds botUserOdds = botUserOddsService.getOneBy(player.getBotUserId(),lmId,lotterySetting.getId(),lotteryType);
        if(null == botUserOdds){
        ...
        for (BuyRecord3DVO cvo : lsDataList) {
            ...
            if ("6".equals(lmId)) { //1D
                if (lotterySetting.getTypeId() == 2) {
                    //猜1D
                    isXian = true;
                    if (cvo.getBuyCode().length() > 1) {
                        codeList = Arrays.asList(cvo.getBuyCode().split(""));
                    } else {
                        codeList.add(cvo.getBuyCode());
                    }
                    value = cvo.getBuyCode();
                    //String[]
    ...
    public Map<String,Object> dxOrJoOrC3tOrTljV2(Player player, LotteryMethod lotteryMethod, LotterySetting lotterySetting,
        ...
        Map<String,Object> resultMap = new HashMap<>();
        String uid = player.getId();
        BotUserOdds userOdds = botUserOddsService.getOneBy(player.getBotUserId(),lotteryMethod.getId(),lotterySetting.getId(),lotteryType);
        if(null == userOdds){
            resultMap.put("errcode", -1);
            resultMap.put("errmsg", "操作失败:赔率参数错误");
            return resultMap;
    ...
    public Map<String,Object> codesBatchBuy(Player player, List<BuyRecord3DVO> codeList, Draw draw, Integer codesFrom,
        ...
        for(String lmId:lmIdSet) {
            ...
            for (String lsTypeId : lsTypeIdSet) {
                ...
                BotUserOdds ar = botUserOddsService.getOneBy(player.getBotUserId(), lmId,lotterySetting.getId());
                if (null == ar) {
                    resultMap.put("errcode", -1);
                    resultMap.put("errmsg", "paramError");
                    return resultMap;
        ...
        for(String lmId:lmIdSet){
            ...
            //总金额
            BigDecimal lmTotalMoney = lmDataList.stream().map(item->item.getBuyMoney().multiply(new BigDecimal(item.getBuyAmount()))).reduce(BigDecimal.ZERO,BigDecimal::add);
            int zongbishu = lmDataList.stream().mapToInt(item->item.getBuyAmount()).sum();
    ...
    public Map<String,Object> hs(Player player, LotteryMethod lotteryMethod, LotterySetting lotterySetting,
        ...
        try {
            ...
            BotUserOdds userOdds = botUserOddsService.getOneBy(player.getBotUserId(),lotteryMethod.getId(),lsId,lotteryType);
            if(null == userOdds){
                resultMap.put("errcode", -1);
                resultMap.put("errmsg", "操作失败:赔率参数错误");
                return resultMap;
            ...
            for (BuyRecord3DVO cvo : lsDataList) {
                ...
                buyRecord.setDrawId(String.valueOf(draw.getDrawId())); //回填期号，用于分库分表
                buyRecord.setBuyCodes(cvo.getValue());
                buyRecord.setParam2(cvo.getValue());
