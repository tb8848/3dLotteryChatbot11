...
//    }
    ...
    @SneakyThrows
    public static void style0(String lotteryName,List<Draw> drawList,String filePath,String fileName) {
        List<Cell> headCells = new ArrayList<>();
        /// C TYPE
        headCells.add(new Cell(1, 1, 90, 1).setTextAlign(true).setContent("["+lotteryName+"]集数"));
        headCells.add(new Cell(1, 2, 90, 1).setTextAlign(true).setContent("时间"));
        headCells.add(new Cell(1, 3, 30, 1).setTextAlign(true).setContent("---"));
        headCells.add(new Cell(1, 4, 30, 1).setTextAlign(true).setContent("结果"));
        headCells.add(new Cell(1, 5, 30, 1).setTextAlign(true).setContent("---"));

        List<List<String>> tableRowContents = new ArrayList<>();
        for(Draw draw : drawList){
            if(StringUtil.isNotNull(draw.getDrawResult())){
                String[] resultList = draw.getDrawResult().split("");
                List<String> a = Stream.of(draw.getDrawId()+"", DateUtil.format(draw.getOpenDateTime(),"yyyy-MM-dd"),resultList[0],resultList[1],resultList[2]).collect(Collectors.toList());
...
//        int[] mergeColumns = new int[]{2,3,4};
        String path = filePath+fileName;
        Table table = new Table().setCellFont(new Font("宋体", Font.BOLD, 15))
                .setHeadCells(headCells).setHeaderFont(new Font("宋体", Font.BOLD, 15))
                .setHeaderBackGroundColor(Color.decode("#238af7")).setFontColor(Color.white)
...
//    }
    ...
    private static HashMap<Integer, Integer> getHeadRowHeight(Map<Integer, List<Cell>> rows, Table table) {
        ...
        return rows.keySet().stream().map(row1 -> {
            ...
            return new Pair<Integer,Integer>(row1,table.getRowHeight() + appendHeight);
    ...
    private static Pair<Integer, Integer> dealRowHeight(List<List<String>> tableRowContents, Table table, int row) {
        ...
        return new Pair<Integer, Integer>(row, table.getRowHeight() + appendHeight);
    }
    ...
    public static BufferedImage starDrawTable(int headRow, List<Cell> tableCells, Table table, int tableHeight) throws IOException {
        ...
        {
            Color cellBgColor = Color.decode("#d5dee7");
            if(i%2==0){
                cellBgColor = Color.decode("#ebeced");
            }
            //表格最后一行画横线
            if (i == allRows + 1) {
...
//                }
            ...
            if (i <= headRow) {
                for (int h = 0; h < perRowCells.size(); h++) {
                    ...
                    coreMethod(graphics, table.getHeaderFont(), lastCellInRow, cell,table.getFontColor());
                }
                return;
            }
            for (int j = 0; j < perRowCells.size(); j++) {
                ...
                if(cell.getContent().length()==1){
                    Color[] colorsArr = new Color[]{Color.red,Color.GREEN,Color.black,Color.cyan,Color.blue};
                    int idx = new Random().nextInt(colorsArr.length);
                    fontColor = colorsArr[idx];
                }
                graphics.setColor(cellBgColor);
                graphics.fillRect(cell.getX(), cell.getY(), cell.getWidth(), cell.getHeight());
                coreMethod(graphics, table.getCellFont(), lastCellInRow, cell,fontColor);
            }
        });
        graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
     ...
     * @param cell          要绘制的单元格
     */
    private static void coreMethod(Graphics2D graphics, Font font, boolean lastCellInRow, Cell cell,Color fontColor) {
